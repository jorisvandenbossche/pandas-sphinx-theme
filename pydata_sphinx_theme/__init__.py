...
def add_toctree_functions(app, pagename, templatename, context, doctree):
    ...
    def get_nav_object2(maxdepth=None, collapse=True, includehidden=True, **kwargs):
    ...
    def get_active_top_level_page():
        ...
        kwargs: key/val pairs
            Passed to the `toctree` Sphinx method
        """
        toc_sphinx = context["toctree"](maxdepth=1, collapse=True, includehidden=True)
        soup = bs(toc_sphinx, "html.parser")
        ...
        nav_object = soup_to_python(soup, only_pages=True)
        active_nav = [nav for nav in nav_object if nav["active"]]
        if active_nav:
            return active_nav[0]["title"]
        else:
            return "Select language"

    context["get_nav_object"] = get_nav_object
    context["get_nav_object2"] = get_nav_object2
    context["get_active_top_level_page"] = get_active_top_level_page
